...
export default class Canvas {
    ...
    constructor(width, height) {
        ...
        this.stage.stage.on('mousedown touchstart', e => {
            ...
            if (pos) {
                ...
                if (this.mode != "emit" && this.currentAnimation) {
    ...
    add_frame(bodyParts) { 
        // 【改善】対象が選ばれていない（currentAnimationが無い）場合は安全に処理を止める
        if (!this.currentAnimation || !bodyParts) {
            console.warn("トラッキング対象が選択されていません。Selectで対象を選んでください。");
            return; 
        }
    ...
    undo() {
        ...
        if (line) {
            ...
            // もし描画中の線（currentFrame）が全て無くなったら、UIを初期状態に戻す
            if (this.currentFrame.length === 0) {
                // app.js側の disableAll() や resetUI() などのグローバル関数を呼ぶのが理想ですが、
                // 取り急ぎ安全に主要ボタンだけを無効化します
                document.getElementById('frame_button').setAttribute('disabled', 'true');
                document.getElementById('save_button').setAttribute('disabled', 'true');
                document.getElementById('emit_button').setAttribute('disabled', 'true');
                document.getElementById('action_dropdown').setAttribute('disabled', 'true');
                
                // Select機能は再度使えるようにしておく
